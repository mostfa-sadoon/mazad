...
<div id="content" class="page-content sign_in-page">
	<div class="page_form">
		<div class="container">
			...
			<h2 class="title_sec">Enter Your OTP</h2>
			<div class="form-sign">
				...
				<form  class="digit-group otp_form" data-group-name="digits" data-autosubmit="false" autocomplete="off">
					...
					<br>
					<div class="foot-btns">
						<a href="#" id="verifPhoneNum" onclick="onSignInSubmit(event);" class="form-control btn btn-primary">CONFIRM</a>
	...
	function onSignInSubmit(e) {
		//$('#verifPhoneNum').on('click', function (e) {
			...
			if (code == '' || code == undefined || code == null) {
				$('#verifPhoneNum').text('يجب ادخال الكود والتأكيد مره اخرى');
				$('#verifPhoneNum').removeAttr('disabled');
			}

			var credential = firebase.auth.PhoneAuthProvider.credential(localStorage.getItem("verificationId") , code);

			firebase.auth().signInWithCredential(credential).then((data) => {

				console.log(data);
				console.log('تم التاكيد من رقم الهاتف، سجل الدخول الان');
				window.location.href = "/new-password/" + "{{$country_code}}" + "/" + "{{$phone}}" + "/" + "{{$reset_password_token}}"

			}).catch((err) => {
				console.log(err.message);	// Token Expired
				$('#verifPhoneNum').text('يجب ادخال كود صحيح والتأكيد مره اخرى');
				setTimeout(() => {
					$('#verifPhoneNum').text('حاول مره اخرى');
					$('#verifPhoneNum').removeAttr('disabled');
				}, 5000);
			})
